...
export default function ArtisticIdentities() {
  ...
  const handleEdit = (e) => {
    ...
    fetch(`/api/artistic-identities/${editingId}`, {
      method: "PUT",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify(payload),
    })
  ...
  const onDeleteAccept = () => {
    ...
    fetch(`/api/artistic-identities/${pendingDeleteId}`, { method: "DELETE" })
      .then((r) => r.json())
      .then((res) => {
        if (res?.status === 200) {
  ...
  return (
    <div className="flex flex-col user_table">
      <h4 className="page_title">Artistic Identities</h4>
