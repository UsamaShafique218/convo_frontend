...
export default function PrimaryMediums() {
  ...
  const handleEdit = (e) => {
    ...
    fetch(`/api/primary-mediums/${editingId}`, {
      method: "PUT",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify(payload),
    })
  ...
  const onDeleteAccept = () => {
    ...
    fetch(`/api/primary-mediums/${pendingDeleteId}`, { method: "DELETE" })
      .then((r) => r.json())
      .then((res) => {
        if (res?.status === 200) {
  ...
  return (
    <div className="flex flex-col user_table">
      <h4 className="page_title">Primary Mediums </h4>
