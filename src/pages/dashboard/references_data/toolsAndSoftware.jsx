...
export default function ToolsAndSoftware() {
  ...
  const handleEdit = (e) => {
    ...
    fetch(`/api/tools-and-software/${editingId}`, {
      method: "PUT",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify(payload),
    })
  ...
  const onDeleteAccept = () => {
    ...
    fetch(`/api/tools-and-software/${pendingDeleteId}`, { method: "DELETE" })
      .then((r) => r.json())
      .then((res) => {
        if (res?.status === 200) {
  ...
  return (
    <div className="flex flex-col user_table">
      <h4 className="page_title">Tools And Software</h4>
