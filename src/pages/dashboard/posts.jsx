...
export default function Posts() {
  ...
  // ✅ use proxy path here instead of full IP
  const fetchPosts = () => {
    setLoading(true);
    fetch("/api/posts")
      .then((res) => res.json())
    ...
    fetch("/api/posts", {
      ...
      .then((res) => {
        if (res?.status === 201) {
          ...
          setCategory("");
          setIsActive(true);
          fetchPosts(); // ✅ refresh with proxy
        } else {
          toast.current?.show({
            severity: "error",
            summary: "Error",
            detail: res?.message || "Failed to create",
            life: 3000,
          });
        }
      });
      ...
      .then((res) => {
        if (res?.status === 200) {
          ...
          toast.current?.show({
            severity: "error",
            summary: "Error",
            detail: res?.message || "Failed to update",
            life: 3000,
          });
        }
      });
  ...
  return (
    <div className="flex flex-col user_table">
      <h4 className="page_title">Posts </h4>
