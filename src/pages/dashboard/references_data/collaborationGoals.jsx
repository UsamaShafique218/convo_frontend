...
export default function CollaborationGoals() {
  ...
  const handleEdit = (e) => {
    ...
    fetch(`/api/collaboration-goals/${editingId}`, {
      method: "PUT",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify(payload),
    })
  ...
  const onDeleteAccept = () => {
    ...
    fetch(`/api/collaboration-goals/${pendingDeleteId}`, { method: "DELETE" })
      .then((r) => r.json())
      .then((res) => {
        if (res?.status === 200) {
  ...
  return (
    <div className="flex flex-col user_table">
      <h4 className="page_title">Collaboration Goals </h4>
