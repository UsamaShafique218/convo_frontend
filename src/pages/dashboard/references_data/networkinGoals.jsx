...
export default function Industries() {
  ...
  const processedData = useMemo(() => {
    ...
    let filtered = data.filter((item) => {
      ...
      return item.name.toLowerCase().includes(q) || status.includes(q);
    });
    ...
    fetch("/api/networking-goals", {
      ...
      .then((res) => {
        if (res?.status === 201 || res?.status === 200) {
          toast.current?.show({ severity: "success", summary: "Created", detail: "Industry added", life: 2500 });
          setShowModal(false);
          setName("");
          setIsActive(true);
          fetchIndustries();
  ...
  const handleEdit = (e) => {
    ...
    fetch(`/api/networking-goals/${editingId}`, {
      method: "PUT",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify(payload),
    })
      ...
      .then((res) => {
        if (res?.status === 200) {
          toast.current?.show({ severity: "success", summary: "Updated", detail: "Industry updated", life: 2500 });
  ...
  const onDeleteAccept = () => {
    ...
    fetch(`/api/networking-goals/${pendingDeleteId}`, { method: "DELETE" })
      .then((r) => r.json())
      .then((res) => {
        if (res?.status === 200) {
          toast.current?.show({ severity: "info", summary: "Deleted", detail: "Industry removed", life: 2500 });
          fetchIndustries();
        } else {
  ...
  return (
    <div className="flex flex-col user_table">
      <h4 className="page_title">Networking Goals</h4>
          ...
          <table className="w-full min-w-[723px] table_main">
            <thead>
              <tr>
                ...
                <th className="border-b border-blue-gray-50 py-3 px-5 text-left text-[11px] font-bold uppercase text-blue-gray-400">Actions</th>
              </tr>
            </thead>
            ...
            <tbody>
              {loading ? (
                <tr>
                  <td colSpan={3} className="py-4 px-5">Loading...</td>
                </tr>
              ) : currentData.length === 0 ? (
                <tr>
                  <td colSpan={3} className="py-4 px-5">No records found.</td>
                </tr>
              ) : (
                currentData.map((row) => (
                  <tr key={row._id}>
                    <td className="py-[10px] px-5 border-b">{row.name}</td>
                    <td className="py-[10px] px-5 border-b">{row.isActive ? "Active" : "Inactive"}</td>
      ...
      <div className="flex items-center justify-between border-t pt-2 pagination_main">
        ...
        <div className="flex items-center gap-2">
          <button onClick={() => setCurrentPage((p) => Math.max(p - 1, 1))} disabled={currentPage === 1} className="px-2 py-1 rounded border text-gray-600 disabled:opacity-30">
            &lt;
          </button>
          ...
          <button onClick={() => setCurrentPage((p) => Math.min(p + 1, totalPages))} disabled={currentPage === totalPages} className="px-2 py-1 rounded border text-gray-600 disabled:opacity-30">
            &gt;
          </button>
        </div>
        ...
        <div className="popup_main" aria-hidden="true">
          <div className="popup_main_inner">
            <div className="popup_header">
              <h3>Add Industry</h3>
              <div className="popup_icon" onClick={() => setShowModal(false)}>
                <button>✕</button>
              </div>
            </div>
              ...
              <form className="space-y-4" onSubmit={handleCreate}>
                <div className="form_row">
                  <div className="form_row_li w-full">
                    ...
                    <input type="text" className="form-control" placeholder="Enter industry name" value={name} onChange={(e) => setName(e.target.value)} required />
                  </div>
                </div>
                <div className="form_row">
                ...
                <div className="popup_btns">
                  <button type="button" className="all_btn gray_color" onClick={() => setShowModal(false)}>Cancel</button>
      ...
      {showEditModal && (
        <div className="popup_main" aria-hidden="true">
          <div className="popup_main_inner">
            <div className="popup_header">
              <h3>Edit Industry</h3>
              <div className="popup_icon" onClick={() => setShowEditModal(false)}>
                <button>✕</button>
              </div>
            </div>
            <div className="p-4 popup_body">
