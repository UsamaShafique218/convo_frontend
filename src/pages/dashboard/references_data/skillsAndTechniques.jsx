...
export default function SkillsAndTechniques() {
  ...
  const fetchIndustries = () => {
    setLoading(true);
    fetch("/api/skills-and-techniques")
      .then((res) => res.json())
      .then((res) => {
  ...
  const handleEdit = (e) => {
    ...
    fetch(`/api/skills-and-techniques/${editingId}`, {
      method: "PUT",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify(payload),
    })
  ...
  const onDeleteAccept = () => {
    ...
    fetch(`/api/skills-and-techniques/${pendingDeleteId}`, { method: "DELETE" })
      .then((r) => r.json())
      .then((res) => {
        if (res?.status === 200) {
  ...
  return (
    <div className="flex flex-col user_table">
      <h4 className="page_title">Skills and Techniques </h4>
            ...
            <div className="p-4 popup_body">
              <form className="space-y-4" onSubmit={handleCreate}>
                ...
                <div className="popup_btns">
                  <button type="button" className="all_btn gray_color" onClick={() => { setShowModal(false); setName(""); setCategory(""); setIsActive(true); }}>Cancel</button>
